(function () {
  Vue.component("reports-v3-programs--evaluations--instructor-recommendation-tags", {
    props: {
      programs: {
        type: Array,
        default: function () {
    ...
    methods: {
      firstValue(row, field) {
        const keys = [
          field,
          `programs__instructor_eval_rec_tags__${field}`,
          `instructor_eval_rec_tags__${field}`,
          `ai_summary__${field}`,
          `programs__instructor_eval_rec_tags__x_ai_summary__${field}`,
          `instructor_eval_rec_tags__x_ai_summary__${field}`
        ];
    ...
    template: `
      <div style="margin-top:18px;">
        <div v-if="!selectedProgramCode" class="btech-card btech-theme" style="padding:16px;">
          <div style="font-weight:600; margin-bottom:4px;">Instructor Recommendation Tags</div>
          <div class="btech-muted">Select a program in Filters to view its instructor recommendation tags.</div>
        </div>

        <div v-else-if="loading" class="btech-card btech-theme" style="padding:16px;">
          <div class="btech-muted">Loading instructor recommendation tags...</div>
        </div>

        <div v-else-if="error" class="btech-card btech-theme" style="padding:16px; border-color:#fecaca; background:#fef2f2;">
          <div style="font-weight:600; margin-bottom:4px;">Instructor Recommendation Tags Error</div>
          <div class="btech-muted">{{ error }}</div>
        </div>

        <div v-else-if="!tagRows.length" class="btech-card btech-theme" style="padding:16px;">
          <div style="font-weight:600; margin-bottom:4px;">Instructor Recommendation Tags</div>
          <div class="btech-muted">No instructor recommendation tags match the current filters.</div>
        </div>
        ...
        <reports-v3-table
          ...
          :default-sort-dir="-1"
          empty-message="No instructor recommendation tags match the current filters."
        />
      </div>
    `
  });
