// program-grduation.js (slim, uses window.COMPLETION)
Vue.component('reports-program-graduates', {
  props: {
    year: { type: [Number, String], required: true },
    anonymous: { type: Boolean, default: false },
    projectedNonGraduates: { type: [Number], default: 0 },
    students: { type: Array, default: () => ([]) },
    graduationRate: { type: Number, default: null },
  ...
  data() {
    ...
    return {
      ...
      tableTickActive: 0,
      tableTickFinished: 0,
      whatIfDrops: Math.max(0, Number(this.projectedNonGraduates) || 0)
  ...
  watch: {
    ...
    students(newVal, oldVal) {
      if (newVal !== oldVal) {
        this.whatIfDrops = Math.max(0, Number(this.projectedNonGraduates) || 0);
      }
    }
  },
...
projectedGreenPctText() {
  const r = this.projectedGraduationRateGreen;
  return Number.isFinite(r) ? (r * 100).toFixed(1) + '%' : 'n/a';
    ...
    pctNowText() {
      const n = this.currentGraduationRate;
    ...
    whatIfPctText() {
      const n = this.whatIfGraduationRate;
      return Number.isFinite(n) ? (n * 100).toFixed(1) + '%' : 'n/a';
    },
    ...
    // bar segments: graduate exiters (faded green) + chosen (bucket colors) + nongraduate exiters + drops
    barSegmentsProjected() {
      const segs = [];
      const { graduates = [], nonGraduates = [] } = this.info || {};
      ...
      for (let i = 0; i < graduates.length; i++) {
        ...
        segs.push({
          ...
          opacity: 0.35,
          title: `${this.displayName(s)}: Gradudated (exiter)`
        });
      }
      ...
      for (let i = 0; i < nonGraduates.length; i++) {
        ...
        segs.push({
          ...
          opacity: 1,
          title: `${this.displayName(s)}: Did not graduate (exiter)`
        });
      }
  ...
  methods: {
    // --- ONLY "duplicated" logic: what-if versions of graduates.js internals ---
    ...
    bucketLabelFromChance(p) {
      ...
      return (
        b === 'green'  ? 'Likely graduate this year' :
        b === 'yellow' ? 'Possible graduate this year' :
    ...
    makeStatusColumn(mode) {
      const desc =
        (mode === 'active')
          ? '● green/yellow/orange/red based on chance_to_graduate. Blank = no score.'
          : '● green = graduate, ● gray = non-graduate exiter.';
    ...
    endDatePillStyle(s, { mode }) {
      ...
      if (mode === 'finished') {
        ...
        return {
          backgroundColor: s?.is_graduate ? this.colors.green : this.colors.gray,
          color: s?.is_graduate ? this.colors.white : this.colors.black,
          opacity: 0.85
        };
      }
...
ratePctText(rate, digits = 1) {
  ...
  <div style="align-items:center; gap:8px; margin-bottom:10px;">
    ...
    Projected: 
    <span class="btech-pill"
          :style="ratePillStyle(projectedGraduationRateGreen)">
      {{ ratePctText(projectedGraduationRateGreen, 1) }}
    </span>
  </div>
        ...
        <div>
          <span style="display:inline-flex; align-items:center; gap:6px;">
            ...
            <button type="button" style="width:22px;height:22px;border-radius:6px;"
              ...
              title="Remove hypothetical non-graduate exiter">−</button>
