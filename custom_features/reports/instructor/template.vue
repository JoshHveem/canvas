  ...
  >
    ...
    <div style="padding:8px; border:1px solid #eef2f7; border-radius:10px;">
      ...
      <div style="height:8px; background:#f3f4f6; border-radius:999px; overflow:hidden;">
        <div
          :style="{
            width: Math.max(0, Math.min(100, Number(grading.perc_graded_with_rubric || 0))) + '%',
            height: '100%',
            background: '#10b981'
          }"
          role="progressbar"
          :aria-valuenow="Number(grading.perc_graded_with_rubric || 0)"
          aria-valuemin="0"
          aria-valuemax="100"
          :aria-label="'Rubric usage ' + (grading.perc_graded_with_rubric || 0) + '%'"
        ></div>
      </div>
      <div style="font-size:11px; color:#6b7280; margin-top:6px;">
