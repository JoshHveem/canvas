...
## Summary
Create a planning record for the ISD Hub editor before implementation begins. This document preserves the current architecture and product decisions while leaving `isd_hub.js` untouched for now.

## Architecture
The ISD Hub editor should use one master editor shell with three compartmentalized editors:
...
## Product Decisions
- Gate the feature by the shared course id config from `window.ISDHubConfig.courseId`, Canvas `ENV.COURSE_ID`, or the current `/courses/[id]` URL.
- Show the editor only on pages that contain the ISD Hub managed-region marker.
- Restrict editing to ISD/admin users.
...
- [ ] Build temporary console snippets that can locate the ISD Hub page content in the DOM.
...
- [ ] Replace only the ISD Hub managed region in the fetched HTML.
...
- [ ] Move only the proven console helpers into `isd_hub.js`.
...
- Implementation will happen later in `isd_hub.js` and any supporting files.
