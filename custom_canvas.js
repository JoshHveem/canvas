  ...
  if (window.self === window.top) { //Make sure this is only run on main page, and not every single iframe on the page. For example, Kaltura videos all load in a Canvas iframe
    if (/^\/courses\/[0-9]+(\/modules){0,1}$/.test(window.location.pathname)) {
      ...
      img.onload = function() {
        ...
        moduleHeader.html(`
          <div style="max-width: 1000px; height:150px; width:100%; display:flex; align-items:center; position:relative; overflow:hidden;">
            <div style="flex-shrink:0; min-width: 40%; padding:0 30px; background:#f5f5f5; height:100%; display:flex; align-items:center; z-index:2;">
              <div style="margin:0; font-size:2rem; line-height:2rem; max-width: 300px; color: #B20B0F; font-family:sans-serif;"><strong>${ENV.current_context.name}</strong></div>
            </div>
